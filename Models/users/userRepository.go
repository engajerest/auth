...
const (
	...
	logintenantresponse="SELECT a.subscriptionid,a.packageid,a.moduleid,a.categoryid,a.subcategoryid,IFNULL(a.validitydate,'') AS validitydate,IF(a.validitydate>=DATE(NOW()), true, false) AS validity,b.modulename FROM tenantsubscription a,app_module b WHERE a.moduleid=b.moduleid  AND  tenantid=? ORDER BY a.subscriptionid ASC "
...
func Tenantresponse(userid int) []Tenant {
	...
	for rows.Next() {
		var t Tenant
		err := rows.Scan(&t.Subscriptionid,&t.Packageid,&t.Moduleid,&t.Categoryid,&t.Subcategoryid,&t.Validiydate,&t.Validity,&t.Modulename)
		if err != nil {
			log.Fatal(err)
		}
		list = append(list, t)
