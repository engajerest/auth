...
const (
	...
	logintenantresponse="SELECT a.subscriptionid,a.packageid,a.moduleid,IFNULL(a.validitydate,'') AS validitydate,IF(a.validitydate>=DATE(NOW()), true, false) AS validity,b.modulename,c.packagename FROM tenantsubscription a,app_module b, app_package c WHERE a.moduleid=b.moduleid AND a.packageid=c.packageid AND  tenantid=? ORDER BY a.subscriptionid ASC "
...
func Tenantresponse(userid int) []Tenant {
	...
	for rows.Next() {
		var t Tenant
		err := rows.Scan(&t.Subscriptionid,&t.Packageid,&t.Moduleid,&t.Validiydate,&t.Validity,&t.Modulename,&t.Packagename)
		if err != nil {
			log.Fatal(err)
		}
		list = append(list, t)
